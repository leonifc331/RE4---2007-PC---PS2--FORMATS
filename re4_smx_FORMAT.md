...
O `.SMX` trabalha junto com o bloco `.SMD`/`cSmd`. Enquanto o `.SMD` define os objetos instanciados, modelos, animações, posição, rotação e escala, o `.SMX` aplica parâmetros extras por objeto: fila de renderização, culling, máscara de luz para o sistema `.LIT`, flags internas, cores de material/especular, deslocamento de textura e um bloco de dados específico do objeto copiado para a área de work em runtime.
...
| `0x04` | `LitSelectMask` | `uint32` | `0x04` | Máscara de seleção das luzes carregadas pelo `.LIT`. Cada bit libera ou bloqueia uma luz runtime de índice `0..31`. |
...
| `0x10` | `Free` | `uint8[116]` | `0x74` | Bloco de parâmetros específicos do objeto. É copiado integralmente para `cObj + 0x3F0..0x463`. Não é a máscara de luz; a seleção direta de luz fica em `LitSelectMask`. |
...
## Relação entre `.SMX` e `.LIT`

O vínculo direto entre o `.SMX` e as luzes do room é o campo `LitSelectMask`, no offset `0x04` de cada entrada `cSmxWork`.

Durante a inicialização do objeto, a rotina `smxInit(cObjScr*, cSmxWork*)` copia esse valor para o objeto/modelo em runtime:

```text
SMX + 0x04  ->  cObj/cModel + 0x210
```

Depois, o `cLightMgr` usa esse valor durante a seleção das luzes ativas do `.LIT`.
...
└─ cLit / cLightWork
   └─ cLightMgr::loadLit()
      └─ cria cLight[] runtime
         └─ cLightMgr::setModel2(cModel*)
            ├─ percorre luzes ativas do cut/room
            ├─ testa LitSelectMask do modelo
            ├─ aplica filtros de tipo, parent, estado e alcance
            └─ grava até 4 luzes selecionadas no modelo
```

### Seleção por bit

Para luzes de índice `0..31`, o runtime faz o teste abaixo:

```c
if (light_index < 32)
{
    if ((model->LitSelectMask & (1u << light_index)) == 0)
        skip_light();
}
```
...
| Bit | Máscara | Luz liberada |
|---:|---:|---|
| `0` | `0x00000001` | Luz `.LIT` runtime índice `0` |
| `1` | `0x00000002` | Luz `.LIT` runtime índice `1` |
| `2` | `0x00000004` | Luz `.LIT` runtime índice `2` |
| `3` | `0x00000008` | Luz `.LIT` runtime índice `3` |
| `4` | `0x00000010` | Luz `.LIT` runtime índice `4` |
| `5` | `0x00000020` | Luz `.LIT` runtime índice `5` |
| `6` | `0x00000040` | Luz `.LIT` runtime índice `6` |
| `7` | `0x00000080` | Luz `.LIT` runtime índice `7` |
| `8..31` | `1 << bit` | Luz `.LIT` runtime índice correspondente |

Observações importantes:

- A máscara controla as primeiras 32 luzes runtime do ambiente atual.
- Luzes com índice `>= 32` não são filtradas por essa máscara de 32 bits.
- A máscara não garante que a luz será aplicada; ela apenas permite que a luz passe para os próximos filtros.
- O modelo recebe no máximo 4 luzes selecionadas no final do processo.
- Mesmo com o bit ligado, a luz ainda pode ser rejeitada por tipo, parent, estado interno, volume de alcance ou teste de colisão/alcance de luz.
- Em modo/debug ou flags globais de render, o jogo possui caminhos que podem ignorar parte do filtro da máscara.

### Relação com o header/cuts do `.LIT`

As rotinas de `cLit` indicam que o `.LIT` organiza luzes por cut/ambiente. A função `cLit::getCut` usa uma tabela de offsets de cuts e `cLit::getMaxLight` percorre os cuts para calcular o maior número de luzes carregáveis em um ambiente.

Estrutura prática do início do `.LIT`, conforme uso do runtime:

```c
struct cLitHeader
{
    uint16_t cut_count;   // 0x00
    uint8_t  version;     // 0x02
    uint8_t  max_light;   // 0x03, atualizado por versionUp/getMaxLight
    uint32_t cut_offset[];// 0x04, offsets relativos/ponteiros de cuts
};
```

Cada cut aponta para uma lista de `cLightWork`. O loader de luz usa entradas de `0x70` bytes para criar objetos `cLight` em runtime.

```text
cLightWork stride = 0x70
```

O índice usado por `LitSelectMask` é o índice runtime da luz depois que o cut/room foi carregado pelo `cLightMgr`, não um offset absoluto dentro do arquivo `.LIT`.

### Valores recomendados para `LitSelectMask`

| Valor | Resultado prático | Uso recomendado |
|---:|---|---|
| `0xFFFFFFFF` | Permite as 32 primeiras luzes runtime. | Valor seguro para objetos comuns. |
| `0x00000000` | Bloqueia as 32 primeiras luzes runtime. | Usar apenas para objetos que devem ignorar iluminação dinâmica/local. |
| `0x00000001` | Permite somente a luz índice `0`. | Testes ou casos muito específicos. |
| `0x00000003` | Permite luzes `0` e `1`. | Casos controlados. |
| `0xFFFFFFFE` | Bloqueia apenas a luz índice `0`. | Ajuste fino. |
| Máscara customizada | Libera apenas os bits ligados. | Preservar/copy-paste de objeto semelhante no mesmo room. |

Para um editor visual, o ideal é exibir `LitSelectMask` de três formas:

1. Hexadecimal de 8 dígitos.
2. Lista de bits `0..31` com checkboxes.
3. Preview textual das luzes liberadas: `Light 0`, `Light 1`, `Light 2`, etc.

## O que realmente é `Free[0x74]`

Apesar do nome usado em ferramentas, `Free[0x74]` não é um campo livre sem função. O runtime copia esse bloco inteiro para uma área interna do objeto:

```text
SMX + 0x10..0x83  ->  cObj + 0x3F0..0x463
```

Essa área é usada por classes específicas de objeto. Portanto, o significado dos bytes depende do tipo real do objeto criado pelo `.SMD`/runtime. Não existe uma única struct global válida para todos os objetos.

Relação com iluminação:

- `Free[0x74]` não seleciona luzes do `.LIT`.
- A seleção direta de luz é feita por `LitSelectMask`.
- `Free[0x74]` pode afetar iluminação apenas de forma indireta, quando controla movimento, matriz, path, estado ou parâmetros do objeto. Se o objeto muda de posição ou matriz, o teste `lightHitCheck(model, light)` pode selecionar luzes diferentes.
- Algumas classes de objeto inicializam estruturas auxiliares de luz/efeito, mas isso não substitui a máscara `LitSelectMask`.

### Mapa runtime do bloco `Free`

| Offset no SMX | Offset no objeto runtime | Tamanho | Interpretação global |
|---:|---:|---:|---|
| `0x10` | `cObj + 0x3F0` | `0x74` | Work area específica do objeto. Preservar byte a byte. |
| `0x10..0x83` | `cObj + 0x3F0..0x463` | `0x74` | Dados brutos; significado depende da classe do objeto. |

### Exemplo de uso por classe: `cObj03`

Uma das classes que usa essa faixa é `cObj03`. Ela lê campos dentro de `cObj + 0x3F0` para path/movimento/debug visual. A tabela abaixo mostra offsets relativos ao início de `Free`.

| Offset em `Free` | Offset runtime | Tipo observado | Uso observado em `cObj03` |
|---:|---:|---|---|
| `+0x04` | `cObj + 0x3F4` | `float32` | Período/limite do parâmetro de path. |
| `+0x08` | `cObj + 0x3F8` | `float32` | Parâmetro atual do path. |
| `+0x0C` | `cObj + 0x3FC` | `float32` | Incremento/velocidade do parâmetro de path por frame. |
| `+0x10` | `cObj + 0x400` | `uint32`/flags | Flag de modo/debug de path. |
| `+0x14` | `cObj + 0x404` | ponteiro/índice runtime | Usado junto com `PathGetMatEm`. Não deve ser tratado como ponteiro persistente no arquivo. |
| `+0x18` | `cObj + 0x408` | ponteiro/índice runtime | Usado junto com `PathGetMatEm`. Não deve ser tratado como ponteiro persistente no arquivo. |

Esse exemplo confirma que o bloco `Free` é uma área de parâmetros por classe de objeto. Em outras classes, os mesmos offsets podem ter outro significado.

### Dados não nulos observados em `Free`

Nos quatro `.SMX` analisados, a maioria das entradas possui `Free` inteiro zerado. Os dados não nulos apareceram apenas em poucos objetos.

| Arquivo | Entrada | `ModelNo` | `Id` | Offsets não nulos em `Free` | Interpretação segura |
|---|---:|---:|---:|---|---|
| `r10d_005.SMX` | `5` | `0x18` | `0x02` | `+0x00`, `+0x04`, `+0x08`, `+0x0C` | Quatro valores `float32`; provável parâmetro específico de objeto/movimento. |
| `r10d_005.SMX` | `6` | `0x19` | `0x02` | `+0x00`, `+0x04`, `+0x08`, `+0x0C` | Quatro valores `float32`; provável parâmetro específico de objeto/movimento. |
| `r10d_005.SMX` | `8` | `0x1B` | `0x02` | `+0x04`, `+0x08`, `+0x0C`, `+0x2C` | Valores `float32`; provável parâmetro específico de objeto/movimento. |
| `r120_006.SMX` | `0` | `0x00` | `0x00` | `+0x04` | Dado packed/flags; não interpreta bem como float. Preservar. |
| `r120_006.SMX` | `2` | `0x02` | `0x00` | `+0x08` | Dado packed/flags; não interpreta bem como float. Preservar. |

Valores extraídos das entradas com `float32` coerente:

| Arquivo | Entrada | Offset | Bytes | `float32` |
|---|---:|---:|---|---:|
| `r10d_005.SMX` | `5` | `+0x00` | `8E C2 F5 3C` | `0.030000` |
| `r10d_005.SMX` | `5` | `+0x04` | `31 C4 31 3D` | `0.043400` |
| `r10d_005.SMX` | `5` | `+0x08` | `5A F5 B9 3C` | `0.022700` |
| `r10d_005.SMX` | `5` | `+0x0C` | `00 BA 40 47` | `49338.000000` |
| `r10d_005.SMX` | `6` | `+0x00` | `96 43 8B 3C` | `0.017000` |
| `r10d_005.SMX` | `6` | `+0x04` | `E0 A5 1B 3D` | `0.038000` |
| `r10d_005.SMX` | `6` | `+0x08` | `DF 4F 0D 3D` | `0.034500` |
| `r10d_005.SMX` | `6` | `+0x0C` | `00 72 48 47` | `51314.000000` |
| `r10d_005.SMX` | `8` | `+0x04` | `04 5F 98 3C` | `0.018600` |
| `r10d_005.SMX` | `8` | `+0x08` | `0A 1D 49 3D` | `0.049100` |
| `r10d_005.SMX` | `8` | `+0x0C` | `00 7B 51 47` | `53627.000000` |
| `r10d_005.SMX` | `8` | `+0x2C` | `B9 5D AA 3D` | `0.083187` |

Regras para editor/repacker:

- Não renomear `Free` como `LitData`; isso causa interpretação errada do formato.
- Exibir como `object_work` ou `free_raw`.
- Oferecer visualização hexadecimal sempre.
- Oferecer visualização opcional como `float32[29]`, apenas para análise.
- Não converter automaticamente valores inválidos como float, porque alguns subcampos podem ser flags, IDs ou dados packed.
- Para entradas novas, usar `00` em todo o bloco ou copiar o bloco de um objeto do mesmo tipo.
- Para roundtrip, preservar os 116 bytes exatamente.
...
`LitSelectMask` é a relação direta do `.SMX` com o sistema de iluminação `.LIT`.

| Propriedade | Valor |
|---|---|
| Offset na entrada | `0x04` |
| Tipo | `uint32` little-endian |
| Destino runtime | `cObj/cModel + 0x210` |
| Função principal | Permitir/bloquear luzes `.LIT` por índice runtime |
| Faixa efetiva | Bits `0..31` |

Valores aceitos:

| Valor | Significado prático |
|---:|---|
| `0xFFFFFFFF` | Libera as 32 primeiras luzes runtime do `.LIT`. |
| `0x00000000` | Bloqueia as 32 primeiras luzes runtime. |
| `1 << n` | Libera somente a luz runtime `n`, para `n = 0..31`. |
| Qualquer `uint32` | Máscara customizada; cada bit ligado permite uma luz. |

Exemplos:

| Máscara | Luzes liberadas |
|---:|---|
| `0x00000001` | `0` |
| `0x00000002` | `1` |
| `0x00000003` | `0, 1` |
| `0x0000000F` | `0, 1, 2, 3` |
| `0xFFFFFFFE` | Todas exceto `0` |
| `0xFFFFFFFF` | Todas as 32 primeiras |
...
- Permitir edição por bitmask `0..31`.
- Mostrar quais luzes runtime estão liberadas pela máscara.
...
- Não interpretar esse campo como cor, ponteiro ou offset.
...
### `Free[116]` / `object_work[0x74]`

Esse bloco começa no offset `0x10` da entrada e possui `0x74` bytes. O nome `Free` é apenas uma convenção de documentação; no runtime ele é copiado para a área de work do objeto.

| Propriedade | Valor |
|---|---|
| Offset na entrada | `0x10` |
| Tamanho | `0x74` bytes / `116` bytes |
| Tipo seguro | `uint8[116]` |
| Destino runtime | `cObj + 0x3F0..0x463` |
| Relação direta com `.LIT` | Nenhuma |
| Relação indireta com luz | Possível, se o bloco alterar posição/matriz/estado do objeto |

Cópia feita pelo runtime:

```text
SMX entry + 0x10  ->  object + 0x3F0
SMX entry + 0x83  ->  object + 0x463
```

O significado interno depende da classe do objeto. Por isso, não existe uma tabela universal de subcampos válida para todos os `ModelNo`.

Valores aceitos:

| Forma | Aceito | Observação |
|---|---|---|
| `uint8[116]` | Sim | Representação correta e segura. |
| Hex string de `232` caracteres | Sim | Forma recomendada em YAML/JSON. |
| `float32[29]` | Apenas visualização | Útil para análise, mas não deve ser assumido como struct global. |
| Tudo `00` | Sim | Valor comum nos arquivos originais. |
| Dados packed/flags | Sim | Alguns objetos usam bytes que não fazem sentido como float. |

Subcampos confirmados para uma classe específica (`cObj03`):

| Offset em `Free` | Offset runtime | Tipo observado | Uso observado |
|---:|---:|---|---|
| `+0x04` | `cObj + 0x3F4` | `float32` | Período/limite de path. |
| `+0x08` | `cObj + 0x3F8` | `float32` | Parâmetro atual do path. |
| `+0x0C` | `cObj + 0x3FC` | `float32` | Incremento/velocidade do path. |
| `+0x10` | `cObj + 0x400` | `uint32` | Flags/modo/debug de path. |
| `+0x14` | `cObj + 0x404` | runtime pointer/index | Usado por `PathGetMatEm`. Não tratar como ponteiro persistente no arquivo. |
| `+0x18` | `cObj + 0x408` | runtime pointer/index | Usado por `PathGetMatEm`. Não tratar como ponteiro persistente no arquivo. |

Dados não nulos observados nas amostras:

| Arquivo | Entrada | `ModelNo` | `Id` | Offsets não nulos em `Free` | Observação |
|---|---:|---:|---:|---|---|
| `r100_005.SMX` | — | — | — | Nenhum | Todas as entradas com `Free` zerado. |
| `r106_005.SMX` | — | — | — | Nenhum | Todas as entradas com `Free` zerado. |
| `r10d_005.SMX` | `5` | `0x18` | `0x02` | `+0x00`, `+0x04`, `+0x08`, `+0x0C` | Valores coerentes como `float32`. |
| `r10d_005.SMX` | `6` | `0x19` | `0x02` | `+0x00`, `+0x04`, `+0x08`, `+0x0C` | Valores coerentes como `float32`. |
| `r10d_005.SMX` | `8` | `0x1B` | `0x02` | `+0x04`, `+0x08`, `+0x0C`, `+0x2C` | Valores coerentes como `float32`. |
| `r120_006.SMX` | `0` | `0x00` | `0x00` | `+0x04` | Dado packed/flags; preservar bruto. |
| `r120_006.SMX` | `2` | `0x02` | `0x00` | `+0x08` | Dado packed/flags; preservar bruto. |

Valores `float32` observados:

| Arquivo | Entrada | Offset em `Free` | Bytes | Valor |
|---|---:|---:|---|---:|
| `r10d_005.SMX` | `5` | `+0x00` | `8E C2 F5 3C` | `0.030000` |
| `r10d_005.SMX` | `5` | `+0x04` | `31 C4 31 3D` | `0.043400` |
| `r10d_005.SMX` | `5` | `+0x08` | `5A F5 B9 3C` | `0.022700` |
...
- Não assumir que todos os subcampos são float.
- Não associar `Free` diretamente a índices de luz do `.LIT`.
- Permitir edição hexadecimal avançada.
- Para novas entradas, iniciar com `0x00` ou copiar o bloco de um objeto semelhante.
- Nome recomendado em ferramentas: `object_work`, `free_raw` ou `object_params_raw`.
...
| `smxInit(cObjScr*, cSmxWork*)` | `0x002A5C88` | Aplica uma entrada `cSmxWork` ao objeto; copia `LitSelectMask` para `+0x210` e `Free[0x74]` para `+0x3F0`. |
| `cLightMgr::setModel2(cModel*)` | `0x0024AC78` | Seleciona luzes do `.LIT` para o modelo; testa `LitSelectMask` em `model + 0x210`. |
| `cLightMgr::loadLit(cLightWork*, uint32)` | `0x0024C258` | Carrega entradas de luz do `.LIT` e cria `cLight` runtime. |
| `cLit::getCut(uint16)` | `0x0024D588` | Resolve o cut/ambiente de luz dentro do `.LIT`. |
| `cLit::getMaxLight()` | `0x0024DEC8` | Calcula o máximo de luzes por cut do `.LIT`. |
| `cLit::versionUp()` | `0x0024D5D8` | Atualiza versões antigas de `.LIT` e parâmetros padrão de luz. |
| `cObj03::move()` | `0x00261280` | Exemplo de consumidor da área `cObj + 0x3F0`, onde `Free` é copiado. |
...
9. `Free[116]` é preservado exatamente e tratado como `object_work` específico por classe.
...
- [ ] Editor hexadecimal e por bits para `LitSelectMask`, com bits `0..31` representando luzes runtime do `.LIT`.
- [ ] Editor RGBA para `MaterialColor` e `SpecularColor`.
- [ ] Editor float para `TexU` e `TexV` com validação de `NaN`/`Inf`.
- [ ] Aba avançada para `Free[116]` em hexadecimal, com visualização opcional como `float32[29]` apenas para análise.
...
O `.SMX` é uma tabela de modificadores de objeto. O arquivo não tem assinatura, usa header fixo de `0x10` bytes e entradas fixas de `0x90` bytes. O campo mais importante para associação é `ModelNo`, que deve bater com o `WorkNo` do `.SMD`. A relação direta com o `.LIT` fica em `LitSelectMask`: cada bit libera ou bloqueia uma luz runtime de índice `0..31`.

O bloco `Free[116]` não é a tabela de luz. Ele é copiado para `cObj + 0x3F0` e funciona como área de parâmetros específica da classe do objeto. Pode afetar iluminação de forma indireta quando altera posição, path, matriz ou estado do objeto, mas a seleção direta das luzes do `.LIT` é feita pela máscara `LitSelectMask`.
